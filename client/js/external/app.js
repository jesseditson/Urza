define(
  ...
  function(Router,viewInfo){
    ...
    for(var viewName in viewObject){
      var route,
          raw = ['main']
      route = viewObject[viewName];
      views[viewName] = viewsArray[viewNum];
      if(!views[viewName]){
        if(console && console.error) console.error("View " + viewName + " failed to load. Attempting to continue.")
        continue
      }
