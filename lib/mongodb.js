...
var MongoDB = module.exports = function(options){
  ...
  if(options.db.secondary && !connection){
    // we have a secondary db, prefer reads there unless told not to
    var replicaConnectString = 'mongodb://';
    if(options.db.auth) replicaConnectString += options.db.secondary.user || options.db.user + ":" + options.db.secondary.pass || options.db.pass + "@";
    replicaConnectString += options.db.secondary.host + ":" + options.db.secondary.port || this.port + "/" + options.db.secondary.name || this.dbName;
    var connectOptions = {
          replset: {
    ...
    connection = mongoose.connectSet(connectString + ", " + replicaConnectString,null,connectOptions,function(err){
      if(err) return console.error("[MONGOOSE] connection error " + error)
    })
  } else {
  ...
  if(module.mapReduce){
    ...
    for(var mr in module.mapReduce){
      ...
      model.statics.mapReduce[mr] = function(query,callback){
        ...
        mongo.connection.db.executeDbCommand(command,function(err,res){
          if(err || !res.documents[0].ok){
            callback(err || documents[0].errmsg);
          } else {
            mongo.connection.db.collection(mr,function(err,collection){
              if(err){
                callback(err);
              } else {
...
MongoDB.prototype.findOperation = function(operation,collection,find,fields,options,callback){
 if(!callback && !options && !fields){
  ...
  var arguments = [find,fields,options,function(e,r){
