...
var files = {
    ...
    viewRoutes = {},
    basePath = 'lib/views/',
    viewsFolder = process.cwd() + '/client/js/lib/views',
    ready = false,
    readyCallbacks = [],
...
var setup = function(){
  ...
  try {
    var views = JSON.parse(fs.readFileSync(process.cwd() + '/client/js/lib/views.json'))
  } catch(e) {
  ...
  walk(viewsFolder,function(file,folderPath){
    ...
    // get unique views
    var allViews = _.union(files.base,files.web,files.mobile),
        missingViews = _.difference(Object.keys(views),allViews)
    if(missingViews.length){
      // probably unnecessary grammar
      var p = missingViews.length == 1 ? 'view' : 'views', w = missingViews.length == 1 ? 'was' : 'were'
      logger.warn('WARNING: the '+p+' "' + missingViews.join('", "') + '" '+w+' defined, but no js files were found for them.')
    }
...
var middleware = module.exports.middleware = function(req,res,next){
  ...
  if(!req.url.match(/^\/js\/lib\/views\.js/)) return next()
  ...
  if(req.isMobile){
    res.send(getViews('mobile'))
  } else {
    res.send(getViews('web'))
  }
}

var getViews = module.exports.getViews = function(type,fail){
  if(viewsFiles[type].length){
    return viewsFiles[type]
  } else if(!fail) {
    // need to generate views files
    viewsFiles.web = defineString.replace('{{{viewArray}}}',JSON.stringify(viewArrays.web)).replace('{{{viewRoutes}}}',JSON.stringify(viewRoutes))
    viewsFiles.mobile = defineString.replace('{{{viewArray}}}',JSON.stringify(viewArrays.mobile)).replace('{{{viewRoutes}}}',JSON.stringify(viewRoutes))
    // try again
    return getViews.call(this,type,true)
  } else {
    throw new Error("Failed attempting to load views for " + type + ".")
  }
}
