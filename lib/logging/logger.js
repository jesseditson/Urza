...
var options = {
      ...
      // Defaults to logging to a console.
  		new (winston.transports.Console)({ "colorize" : true, "level" : "silly", "silent" : false, "timestamp" : true })
  	]
  };
if(config.environment == "production"){
  options.exitOnError = true;
  options.exceptionHandlers = [new (winston.transports.Console)({ "colorize" : true, "level" : "info", "silent" : false, "timestamp" : true })];
}
...
if(config.logFiles){
  logger.add(winston.transports.File,{ "filename" : log_folder + "/log.log", "timestamp" : true, "level" : "info" });
  logger.handleExceptions(new (winston.transports.File)({ "filename": log_folder + "/exceptions.log", "timestamp" : true }));
}
