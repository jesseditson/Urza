...
var doPostMiddlewares = function(err,response,callback,endpoint,params,session,body,index,complete,newErr,newRes){
  ...
  next = doPostMiddlewares.bind(this,err,response,callback,endpoint,params,session,body,index+1,complete)
      ...
      async.forEachSeries(this.middlewares,function(middleware,done){
        ...
        if(!err){
          findRoute.call(this,endpoint,params,session,body,function(err,response){
            // do post middlewares
            doPostMiddlewares.call(this,err,response,callback,endpoint,params,session,body)
          }.bind(this))
        } else {
