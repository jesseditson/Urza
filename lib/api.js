...
Api.prototype.route = function(params,session,body,callback){
  ...
  if(!this.ready) return this.readycallback = this.route.bind(this,params,session,body,callback);
  // otherwise, just call the api.
  var originalParams = Array.prototype.slice.call(params)
  ...
  async.forEachSeries(this.middlewares,function(middleware,done){
    ...
    if(!err){
      findRoute.call(this,endpoint,params,session,body,function(err,response){
        // do post middlewares
        this.doPostMiddlewares.call(this,err,response,callback,endpoint,originalParams,session,body)
      }.bind(this))
    } else {
